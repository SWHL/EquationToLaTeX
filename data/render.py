...
from data.latex2png import Latex, tex2pil
...
def render_dataset(dataset: np.ndarray, unrendered: np.ndarray, args) -> np.ndarray:
    ...
    for batch_offset in tqdm(
        ...
        # print('\n', i, len(math), '\n'.join(math))
        font = font = (
            np.random.choice(args.font) if len(args.font) > 1 else args.font[0]
        )
        dpi = (
            np.random.choice(np.arange(min(args.dpi), max(args.dpi)))
            if len(args.dpi) > 1
            else args.dpi[0]
        )
        if len(valid_math) > 0:
            ...
            for inbatch_idx, order_idx in enumerate(
                ...
                outpath = os.path.join(
                    args.out, "%07d.png" % unrendered[order[order_idx]]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--font", nargs="+", type=str, default="", help="font to use."
    )
    parser.add_argument(
        "-fp",
        "--fonts_path",
    ...
    args.font = args.font if args.font != "" else get_installed_fonts(args.fonts_path)
    print(args.font)
