...
from data.extract_latex import find_math

htmltags = re.compile(r"<(noscript|script)>.*?<\/\1>", re.S)
wikilinks = re.compile(r'href="/wiki/(.*?)"')
wiki_base = "https://en.wikipedia.org/wiki/"
stackexchangelinks = re.compile(
    r"(?:(https:\/\/\w+)\.stack\w+\.com|)\/questions\/(\d+\/[\w\d\/-]+)"
)
...
def parse_stack_exchange(url):
    ...
    linked = list(
        set(
            [l[1] for l in re.findall(stackexchangelinks, text) if url.startswith(l[0])]
        )
    )
...
def recursive_wiki(seeds, depth=4, skip=[], base_url=wiki_base):
    ...
    return recursive_search(
        parse_wiki, start, depth, skip, base_url=base_url, unit=" links"
    )


def recursive_stack_exchange(
    seeds, depth=4, skip=[], base_url=math_stack_exchange_base
):
    """Recursively search through stack exchange questions"""
    start = [s.partition(base_url.split("//")[-1])[-1] for s in seeds]
    return recursive_search(
        parse_stack_exchange, start, depth, skip, base_url=base_url, unit=" questions"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--mode",
        default="auto",
        choices=["auto", "wiki", "math_stack", "physics_stack"],
    ...
    try:
        if args.mode == "physics_stack":
            visited, math = recursive_stack_exchange(
                args.url, base_url=physics_stack_exchange_base
            )
        elif args.mode == "math_stack":
            visited, math = recursive_stack_exchange(
                args.url, base_url=math_stack_exchange_base
            )
        elif args.mode == "wiki":
    ...
    for l, name in zip(
        [visited, math], [f"visited_{args.mode}.txt", f"math_{args.mode}.txt"]
    ):
