...
MAX_CHARS = 3000
dollar = re.compile(
    r"((?<!\$)\${1,2}(?!\$))(.{%i,%i}?)(?<!\\)(?<!\$)\1(?!\$)" % (1, MAX_CHARS)
)
inline = re.compile(
    r"(\\\((.*?)(?<!\\)\\\))|(\\\[(.{%i,%i}?)(?<!\\)\\\])" % (1, MAX_CHARS)
)
equation = re.compile(
    r"\\begin\{(equation|math|displaymath)\*?\}(.{%i,%i}?)\\end\{\1\*?\}"
    % (1, MAX_CHARS),
    re.S,
)
align = re.compile(
    r"(\\begin\{(align|alignedat|alignat|flalign|eqnarray|aligned|split|gather)\*?\}(.{%i,%i}?)\\end\{\2\*?\})"
    % (1, MAX_CHARS),
    re.S,
)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="file to save equations to. If none provided, print all equations.",
    )
    parser.add_argument(
        "--wiki",
        action="store_true",
        help="only look for math starting with \\displaystyle",
    )
    parser.add_argument(
        "--unescape", action="store_true", help="call `html.unescape` on input"
    )
    ...
    from data.demacro import pydemacro
