...
def get_device(args, no_cuda=False):
    ...
    args.gpu_devices = (
        args.gpu_devices
        if args.get("gpu_devices", False)
        else list(range(available_gpus))
    )
    if available_gpus > 0 and not no_cuda:
        device = "cuda:%d" % args.gpu_devices[0] if args.gpu_devices else 0
        assert available_gpus >= len(
            args.gpu_devices
        ), "Available %d gpu, but specified gpu %s." % (
            available_gpus,
            ",".join(map(str, args.gpu_devices)),
        )
        assert (
            max(args.gpu_devices) < available_gpus
        ), "legal gpu_devices should in [%s], received [%s]" % (
            ",".join(map(str, range(available_gpus))),
            ",".join(map(str, args.gpu_devices)),
        )
    return device
...
def gpu_memory_check(model, args):
    ...
    try:
        batchsize = (
            args.batchsize
            if args.get("micro_batchsize", -1) == -1
            else args.micro_batchsize
        )
        for _ in range(5):
            im = torch.empty(
                batchsize,
            ...
            seq = torch.randint(
                0, args.num_tokens, (batchsize, args.max_seq_len), device=args.device
            ).long()
            loss = model.data_parallel(im, device_ids=args.gpu_devices, tgt_seq=seq)
            loss.sum().backward()
    except RuntimeError:
        raise RuntimeError(
            "The system cannot handle a batch size of %i for the maximum image size (%i, %i). Try to use a smaller micro batchsize."
            % (batchsize, args.max_height, args.max_width)
        )
    model.zero_grad()
...
def in_model_path():
    model_path = str(Path(__file__).resolve().parent.parent / "checkpoints")
    saved = os.getcwd()
    # os.chdir(model_path)
    try:
        yield
