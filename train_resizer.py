...
def main(args):
    ...
    def train_epoch(sched=None):
        ...
        for i in dset:
            ...
            if im is not None:
                if (
                    im.shape[-1] > dataloader.max_dimensions[0]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resume", help="path to checkpoint folder", type=str, default=""
    )
    parser.add_argument(
        "--out",
        type=str,
        default="checkpoints/image_resizer.pth",
        help="output destination for trained model",
    )
    parser.add_argument(
